...
class NetworkedLevel:
    ...
    def update(self,delta : float,gametime : float,levelHandler):
        ...
        for i, player in enumerate(self.players):
            ...
            self.movementUpdate(delta,keys,player,gametime,input)
            self.pickupUpdate(player,i)
            self.shootUpdate(player,input,gametime)
            self.lookDirUpdate(i,player,input)
    ...
    def lookDirUpdate(self,i : int,player,input):
        if(input[0]):
    ...
    def shootUpdate(self,player,input,gameTime : float):
    ...
    def movementUpdate(self,delta : float,keys,player,gameTime : float,input : list[bool]):
        #input
        if(input[0]):
